...
//
// ────────────────────────────────────────────────────────────────
//  2) RECHERCHE WEB (DuckDuckGo instant answer API)
// ────────────────────────────────────────────────────────────────
//
void search_web(const char *user_question) {
    ...
    snprintf(command, sizeof(command),
        "curl -s \"https://api.duckduckgo.com/?q=%s&format=json\" "
        "> result.json",
        query);
...
//
// ────────────────────────────────────────────────────────────────
//  4) EXTRACTION DES SNIPPETS (résumés DuckDuckGo)
// ────────────────────────────────────────────────────────────────
//



char *extract_snippets(const char *json_data) {
    cJSON *root = cJSON_Parse(json_data);
    if (!root) return NULL;

    char *snippets = malloc(5000);
    snippets[0] = '\0';

    // 1) Résumé principal
    cJSON *abstract = cJSON_GetObjectItem(root, "AbstractText");
    if (abstract && abstract->valuestring) {
        strcat(snippets, abstract->valuestring);
        strcat(snippets, "\n---\n");
    }

    // 2) Related topics
    cJSON *topics = cJSON_GetObjectItem(root, "RelatedTopics");
    if (topics) {
        for (int i = 0; i < cJSON_GetArraySize(topics); i++) {
            cJSON *item = cJSON_GetArrayItem(topics, i);
            cJSON *text = cJSON_GetObjectItem(item, "Text");
            if (text && text->valuestring) {
                strcat(snippets, text->valuestring);
                strcat(snippets, "\n---\n");
            }
        }
    }
...
void call_openai_api(const char *snippets) {
    const char *api_key = load_api_key();
    char *safe_snippets = escape_json(snippets);

    // buffer très large
    char *command = malloc(20000);

    snprintf(command, 20000,
        "curl -s -X POST https://api.openai.com/v1/chat/completions "
        "-H \"Content-Type: application/json\" "
        "-H \"Authorization: Bearer %s\" "
        "-d '{\"model\": \"gpt-4.1\", \"messages\": [{\"role\": \"user\", \"content\": \"Analyse ces extraits et dis si l'affirmation est VRAIE, FAUSSE ou PAS SÛR : %s\"}]}' "
        "> ai_result.json",
        api_key, safe_snippets);
    ...
    printf("\n✅ Analyse IA terminée. Résultat dans ai_result.json\n");
}

//
// ────────────────────────────────────────────────────────────────
//  6) MAIN : CHAÎNAGE COMPLET
// ────────────────────────────────────────────────────────────────
//
...
int main() {
    ...
    trim(question);
    to_lowercase(question);
    remove_punctuation(question);

    printf("\n Recherche web...\n");
    search_web(question);

    printf(" Lecture du JSON DuckDuckGo ...\n");
    char *json_data = load_file("result.json");

    printf("Extraction des snippets...\n");
    char *snippets = extract_snippets(json_data);

    printf(" Envoi à l'IA...\n");
    call_openai_api(snippets);
...
int main() {
    ...
    // Lire la question depuis question.txt
    char *question_file = load_file("question.txt");
    if (!question_file) {
        printf("Erreur : impossible de lire question.txt\n");
        return 1;
    }
    ...
    snprintf(question, sizeof(question), "%s", question_file);
    snprintf(original_question, sizeof(original_question), "%s", question_file);
    trim(question);

    printf("Question reçue : %s\n", question);  // Pour debug, tu peux l’enlever plus tard
